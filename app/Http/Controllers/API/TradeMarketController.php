...
class TradeMarketController extends Controller
{
    ...
    public function buymarket(Request $request) {
		...
		if ($validator->fails()) {      
			return response()->json(["success" =>false,"result" =>"","message"=>$validator->errors()->first()],$this->successStatus);  
		}
		$buypair = (int)$request->pair;
		$buymarketvolume = (float)$request->quantity;
			...
			if($pair){
				...
				if($pair->type == 1 || $pair->type == 2){
					...
					if($pair->min_buy_amount > 0 && $volume < $pair->min_buy_amount)
					{
						...
						return response()->json(["success" => false,"result" =>"","message"=>"Minimum buy amount ".number_format($pair->min_buy_amount, 8, '.', '')],$this->successStatus); 
					}
					if($pair->max_amount > 0 && $volume > $pair->max_amount)
					{
						return response()->json(["success" =>false,"result" =>"", "message"=>"Maximum buy amount ".number_format($pair->max_amount, 8, '.', '')],$this->successStatus); 
					}
					$balance = 0;
					...
					{
						return response()->json(["success" =>false, "result" => "","message"=>"Total value must be at least ".number_format($pair->min_buy_price, 8, '.', '')],$this->successStatus); 
					}
					DB::beginTransaction();
					try
					{
						...
						if($get_total_val > $balance){
							...
							return response()->json(["success" => false,"result" =>"" ,"message"=>"Your current balance is ".$is_user_balance." ".$pair_details->cointwo." (Your trade volume is ".$volume.")"],$this->successStatus); 
						}
						...
						if($orderbook['status'] == 'success')
						{
							...
							if(isset($result['fills']))
							{
								...
								DB::commit();
								return response()->json(["success" => true,"result" =>"","message"=>"PostTradeSuccessfully"],$this->successStatus); 
							} else{
								$trade->status = 100;
								$trade->status_text = "Operation Cancel";
								$trade->save();

								return response()->json(['success'=>false,'result'=>"","message"=>"Something went wrong, try again later!"]);
							}
						} else {
							...
							return response()->json(["success" => false,"result" =>"","message"=>$message],$this->successStatus); 
						}
					}
					catch(Exception $e)
					{
						//rollback here
						DB::rollBack();
						return response()->json(["success" =>false,"result"=>"","message" => "Something went wrong, try again later!"],$this->successStatus); 
					}
							...
							if ($result->count() > 0) {
								...
								if ($result->sum('remaining') >= $volume) {
									...
									if($obalance <= 0){
										...
										return response()->json(["success" =>false,"result"=>"", "message" => "Insufficient".$pair->cointwo." "."Wallet"],$this->successStatus); 
									}
									//select user balance
									$balance = 0;
									$wallet = Wallet::where([
									...
									$this->AllcoinUpdateBalanceTrack($user,$pair->cointwo,0,$totalprice,$balance,$obalance,'buytrade','Buy Post Market Trade '.$tCurrency,$trade_id);
										...
										DB::commit();
										$data['balance'] = $balance;
										return response()->json(["success" =>true, "result" =>"","message"=>"Post Trade Successfully"],$this->successStatus); 

									} else {
										DB::rollBack();
										return response()->json(["success" =>false, "result" =>"","message"=>"Insufficient"." ".$pair->cointwo],$this->successStatus); 

									}

								} else {
									DB::rollBack();
									return response()->json(["success" =>false, "result" =>"","message"=>"Only Available".display_format($result->sum('remaining'))],$this->successStatus); 
								}
							} else {
								DB::rollBack();
								return response()->json(["success" =>false, "result" =>"","message"=>"Trade not availableyet"],$this->successStatus); 
							}
						} else {
							DB::rollBack();
							return response()->json(["success" =>false, "result" =>"","message"=>"Amount must be".display_format($min_trade_volume)],$this->successStatus); 
						}
					}catch(Exception $e){
						DB::rollBack();
						return response()->json(["success" =>false, "result" =>"", "message"=>"Something went wrong, try again later!"],$this->successStatus); 
				...
				return response()->json(["success" =>false, "result" =>"","message"=>"InvalidTradepair"],$this->successStatus);
			}
		} else {
			return response()->json(["success" =>false, "result" =>"","message"=>"Allfieldsrequired"],$this->successStatus);
	...
	public function sellmarket(Request $request) {		
		...
		$pair = (int)$request->pair;
		$sellmarketvolume = (float)$request->quantity;
		if ($pair != "" && $sellmarketvolume > 0) {
			...
			if($pair){
				...
				if($pair->type == 1 || $pair->type == 2){
					...
					{
						return response()->json(["success" =>false,"result" =>"","message"=>"Minimum sell amount ".number_format($pair->min_sell_amount, 8, '.', '')],$this->successStatus);
					}

					if($pair->max_amount > 0 && $volume > $pair->max_amount)
					{
						return response()->json(["success" =>false, "result" => "","message"=>"Maximum sell amount ".number_format($pair->max_amount, 8, '.', '')],$this->successStatus);
					}
					$balance = 0;
					...
					if($balance < $volume)
					{
						return response()->json(["success" =>false, "result" =>"","message"=>"Total value must be at least ".number_format($volume, 8, '.', '')." ".$pair->coinone],$this->successStatus);
					}
					DB::beginTransaction();
					try
					{						
						...
						if ($volume > 0 && $min_trade_volume <= $volume) {
							if ($result->count() > 0) {
								if ($result->sum('remaining') >= $volume) {
									...
									if($obalance <= 0){
										return response()->json(["success" =>false,"result" =>"","message"=>"Insufficient"." ".$pair->coinone ."wallet!"],$this->successStatus);
									}
										$require_volume = $volume;
										$totalprice = 0;
										foreach ($result as $res) {
									...
									DB::rollBack();
									return response()->json(["success" =>false,"result" =>"","message"=>"Onlyavailable".display_format($result->sum('remaining'))],$this->successStatus);
								}
							} else {
								DB::rollBack();
								return response()->json(["success" =>false, "result" =>" ","message"=>"Tradenotavailableyet"],$this->successStatus);
							}
						} else {
							DB::rollBack();
							return response()->json(["success" =>false, "result" =>"","message"=>"Amount must be".display_format($min_trade_volume)],$this->successStatus);
