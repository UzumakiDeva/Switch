...
<body>
	<div id="marketdepth" class="marketchart">
...
var title = chart.plotContainer.children.push(am5.Label.new(root, {
  text: "Price ({{$selectPair->coinone}}/{{$selectPair->cointwo}})",
  fontSize: 20,
  fontWeight: "400",
...
// Data loader
function loadData() {
  am5.net.load("{{ url('marketdepthchart/'.$selectPair->id)}}").then(function(result) {
  	//loadOrderbookAM(result);
	...
	var selectPair = "$cpair";
	var websocket = new WebSocket("wss://ws.sfox.com/ws"); 
	var ofeed =  "orderbook.net.{{ $selectPair->symbol }}";	
