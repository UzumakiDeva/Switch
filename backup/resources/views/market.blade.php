                ...
                <div class="row">
									...
									<div class="marketpricebox">
										<div class="mrkpricetable">
											<div>
												...
												<h4 class="h4 @if($tabtrade->hrchange >= 0 ) t-green @else t-red @endif">{{ $tabtrade->close }} </h4>
											</div>
											<div class="text-right"> <img src="images/graphs.svg" /> </div>
										</div>
										<hr>
										<h6 class="h6">24h change : <span class="@if($tabtrade->hrchange >= 0 ) t-green @else t-red @endif smllspan">{{ $tabtrade->hrchange }}%</span><br/>Volume : {{ $tabtrade->hrvolume }} {{$tabtrade->cointwo}}</h6>
									 </div>
								</div>
								@empty
								<div class="panelcontentbox">
						...
						<div class="col-lg-12 col-md-12 col-sm-12 col-12">
							...
							@foreach ($marketpairs as $key => $value)
								<li class="nav-item"><a class="nav-link @if($key == 'USDT') active @endif" data-bs-toggle="tab" href="#{{ $key }}market" data-bs-target="#{{ $key }}market">{{ $key }} Market</a></li>
								@endforeach
								...
								@foreach ($marketpairs as $key => $value)
									<div id="{{ $key }}market" class="tab-pane @if($key == 'USDT') active @endif">
											...
											<table class="table sitetable">
												...
												<tbody>
													...
													<td><img src="{{ url('images/color/'.$trade->coinonedetails['image']) }}" class="coinlisticon">{{$trade->coinone.'/'.$trade->cointwo}}</td>
														...
														<td class="open_{{$trade->coinone.$trade->cointwo}}">{{ $trade->open }}</td>
														<td><span class="@if($trade->hrchange >= 0 ) t-green @else t-red @endif price_change_{{$trade->coinone.$trade->cointwo}}">{{ $trade->hrchange }}</span></td>
														<td class="high_{{$trade->coinone.$trade->cointwo}}">{{ $trade->high }}</td>
														<td class="low_{{$trade->coinone.$trade->cointwo}}">{{ $trade->low }}</td>
