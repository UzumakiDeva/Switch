@php $title = $coinone.$cointwo." | ".$coinone." Buy |".$coinone." Sell";
$atitle ="trade";
$cpair = $coinone.$cointwo;
@endphp
						...
						<div class="livepricelist">
							<div class="livepricemobile">
								<ul class="livepricenavbg">
									<li>
										<a class="livepricet coinlisttable"> <img src="{{ url('images/color/'.$selectPair->coinonedetails['image']) }}" class="coinlisticon" />
					     ...
					     <span class="d-t"><i class="fa fa-angle-down" aria-hidden="true"></i></span>	</a>
											<div class="dropdown-menu">
											@forelse($trades as $trade)
												<a class="dropdown-item" href="{{url('trades/'.$trade->coinone.'_'.$trade->cointwo)}}"><img src="{{ url('images/color/'.$trade->coinonedetails['image']) }}" class="coinlisticon">{{$trade->coinone.'/'.$trade->cointwo}}</a>
											@empty
									...
									<li><a class="livepricet">Last price<br><span class="@if($selectPair->hrchange >= 0) t-green @else t-red @endif last_price_{{$cpair}}" id="lastprice">{{ display_format($selectPair->close,$selectPair->cointwo_decimal) }}</span></a></li>
									...
									<li><a class="livepricet">High price<br><span class="@if($selectPair->hrchange >= 0) t-green @else t-red @endif high_{{$cpair}}" id="opneprice">{{ display_format($selectPair->high,$selectPair->cointwo_decimal) }}</span></a></li>
									<li><a class="livepricet">Low Price<br><span class="@if($selectPair->hrchange >= 0) t-green @else t-red @endif low_{{$cpair}}">{{ display_format($selectPair->low,$selectPair->cointwo_decimal) }}</span></a></li>
									<li><a class="livepricet">24H change<br><span class="@if($selectPair->hrchange >= 0) t-green @else t-red @endif price_change_{{$cpair}}">{{ display_format($selectPair->hrchange,2) }}%</span></a></li>
								</ul>
							</div>
						</div>
							...
							<div class="table-responsive" data-simplebar>
								<table class="table sitetable">
									...
									@forelse($trades as $trade)
										<tr>
											<td>
												<a class="t-white" href="{{url('trades/'.$trade->coinone.'_'.$trade->cointwo)}}">
													<img src="{{ url('images/color/'.strtolower($trade->coinone).'.svg') }}"  class="coinlisticon">{{$trade->coinone.'/'.$trade->cointwo}}
												</a>
											</td>
											<td class="last_price_{{$trade->coinone.$trade->cointwo}}">{{ $trade->close }}</td>
											<td>
												<span class="@if($trade->hrchange >= 0 ) t-green @else t-red @endif price_change_{{$trade->coinone.$trade->cointwo}}">{{ $trade->hrchange }}% <i class="fa fa-arrow-up"></i></span>
											</td>
											<td class="quote_{{$trade->coinone.$trade->cointwo}}">{{ $trade->hrvolume }}</td>
										</tr>
									@empty
										<tr>
											<td><img src="{{ url('images/color/btc.svg') }}" class="coinlisticon">{{$coinone}}/{{ $cointwo }}</td>
											<td>256.36259548</td>
							...
							<div class="orderbookscroll">
								...
								<div class="livepricebox" id="livepricebox">
									<table class="table sitetable">
										<thead>
											<tr>
												<th><span class="@if($trade->hrchange >= 0 ) t-green @else t-red @endif last_price_{{$cpair}}">{{ display_format($selectPair->close,$selectPair->cointwo_decimal) }}</span></th>
												<th class="text-right"><span class="price_change_{{$cpair}}">{{ display_format($selectPair->hrchange,2) }}% </span><i class="fa fa-signal"></i></th>
											</tr>
										</thead>
									</table>
								</div>
						   ...
						   <div class="none rulesnotes">
							...
							<div class="orderformbg">
								...
								<div class="buyselltabbg">
									<ul class="nav nav-tabs orderfrmtab buyselltab" role="tablist">
										<li class="nav-item"><a class="nav-link active" id="buytab" role="tab" data-bs-toggle="tab" href="#buy" data-bs-target="#buy">Buy</a></li>
										<li class="nav-item"><a class="nav-link" id="selltab" role="tab" data-bs-toggle="tab" href="#sell" data-bs-target="#sell">Sell</a></li>
									</ul>
								</div>
								<!--new!-->
							...
							<div class="tab-content">
								...
								<div id="limit" class="tab-pane fade in show active">
									<div class="orderformbg1">
										<div class="buyorderform">
											...
											<form class="siteformbg" id="buylimit">
												...
												<div class="form-group"><span> Available Balance: <span class="balanceTwo">{{ $cointwobalance }}</span> {{$cointwo}}</span></div>
												<div class="form-group">
													...
													<input type="hidden" placeholder="" class="input-xlarge" id="buypair" name="buypair" value="{{ $selectPair->id }}" >
														<input id="buyprice" type="number" placeholder="" onkeypress="return AvoidSpace(event)" class="form-control" name="buyprice" step="0.000001" min="0" max="100000000" >
														<div class="input-group-append"> <span class="input-group-text">{{ $cointwo }}</span> </div>
													</div>
												</div>
												<div class="form-group">
													<div class="input-group">
														<div class="input-group-prepend"> <span class="input-group-text">Amount</span> </div>
														<input id="buyvolume" type="number" onkeypress="return AvoidSpace(event)" class="form-control" name="buyvolume" step="0.000001" min="0" max="100000000" placeholder="">
														<div class="input-group-append"> <span class="input-group-text">{{$coinone}}</span> </div>
													</div>
												</div>	
												<div class="form-group">
													<div class="input-group">
														<div class="input-group-prepend"> <span class="input-group-text">Total</span> </div>
														<input type="text" class="form-control" placeholder="" id="buytotal" >
														<div class="input-group-append"> <span class="input-group-text">{{ $cointwo }}</span> </div>
													</div>
												</div>
												<div class="form-group">
													<div class="control-value-box stoplimtboxt">
														...
														<div>
															<div class="row link-div">
																<div class="col" onclick="calculateBuylimitAmount('25')">25%</div>
																<div class="col" onclick="calculateBuylimitAmount('50')">50%</div>
																<div class="col" onclick="calculateBuylimitAmount('75')">75%</div>
																<div class="col" onclick="calculateBuylimitAmount('100')">100%</div>
															</div>
														</div>
													</div>
												</div>
												<div class="form-group fee-formbox">
													<div class="input-group"> <span class="input-group-prepend"> <div class="commision"><span class="feeamt">Leverage comission :</span> <span class="feeamt">{{ $selectPair->buy_trade }}%</span></div>
													<div class="commision"><span class="feeamt">Fee</span> <span class="feeamt" id="buyfees">0.0000000 {{$coinone}}</span></div>
													</span>
												</div>
										</div>
										<div class="ratiobtn">
											...
											<div class="text-center">
												<input type="button" id="buylimit_order" class="btn btn-block sitebtn green-btn" value="Buy ({{$coinone}})" /> </div>
										</div>
										</form>
									</div>
									...
									<form id="sellimit" class="siteformbg">
										...
										<div class="form-group"><span> Available Balance: <span class="balanceOne"> {{ $coinonebalance }} </span>{{$coinone}}</span></div>
											<div class="form-group">
												<div class="input-group">
												<input type="hidden" placeholder="" class="input-xlarge" id="sellpair" name="sellpair" value="{{ $selectPair->id }}" >
												<div class="input-group-prepend"> <span class="input-group-text">Price</span> </div>
													<input type="number" placeholder="Price"  onkeypress="return AvoidSpace(event)" class="form-control" id="sellprice" name="sellprice" required="required" step="0.0001" min="0" max="1000000" value="{{ $selectPair->close }}">
													<div class="input-group-append"> <span class="input-group-text">{{ $cointwo }}</span> </div>
												</div>
											</div>
											<div class="form-group">
												<div class="input-group">
													<div class="input-group-prepend"> <span class="input-group-text">Amount</span> </div> 
													<input type="number" placeholder="" onkeypress="return AvoidSpace(event)" class="form-control" id="sellvolume" name="sellvolume"  required="required" step="0.0001" min="0" max="1000000">
													<div class="input-group-append"> <span class="input-group-text">{{$coinone}}</span> </div>
												</div>
											</div>
											<div class="form-group">
												<div class="input-group">
													<div class="input-group-prepend"> <span class="input-group-text">Total</span> </div>
													<input type="text" class="form-control" placeholder="" id="selltotal" name="selltotal" >
													<div class="input-group-append"> <span class="input-group-text">{{ $cointwo }}</span> </div>
												</div>
											</div>
											<div class="form-group">
												<div class="control-value-box stoplimtboxt">
													...
													<div>
														<div class="row link-div">
															<div class="col" onclick="calculateSelllimitAmount('25')">25%</div>
															<div class="col" onclick="calculateSelllimitAmount('50')">50%</div>
															<div class="col" onclick="calculateSelllimitAmount('75')">75%</div>
															<div class="col" onclick="calculateSelllimitAmount('100')">100%</div>
														</div>
													</div>
												</div>
											</div>
											<div class="form-group fee-formbox">
												<div class="input-group"> <span class="input-group-prepend"> <div class="commision"><span class="feeamt">Leverage comission :</span> <span class="feeamt">{{ $selectPair->sell_trade }}%</span></div>
												<div class="commision"><span class="feeamt">Fee</span> <span class="feeamt" id="sellfees">0.0000000 {{ $coinone }}</span></div>
												</span>
											</div>
									</div>
									<div class="ratiobtn">
										<div class="text-center">
											<input type="button" name="" class="btn btn-block sitebtn red-btn" id="selllimit_order" value="Sell ({{ $coinone }})" /> </div>
									</div>
									</form>
								</div>
							</div>
								...
								<div class="buyorderform">
									...
									<div class="form-group"><span> Available Balance: <span class="balanceTwo">{{ $cointwobalance }} </span>{{$cointwo}}</span></div>
										<div class="form-group">
											<div class="input-group">
											<input type="hidden" placeholder="" class="input-xlarge" id="buypair" name="buypair" value="{{ $selectPair->id }}" >
												<input type="text" class="form-control" placeholder="Price" value="Market Price" disabled="">
												<div class="input-group-append"> <span class="input-group-text">{{ $cointwo }}</span> </div>
											</div>
										</div>
										<div class="form-group">
											<div class="input-group">
												<div class="input-group-prepend"> <span class="input-group-text">Amount</span> </div>
											<input type="number" placeholder="" class="form-control" id="buymarketvolume" name="buymarketvolume" onkeyup="if (/[^0-9.]/g.test(this.value)) this.value = this.value.replace(/[^0-9.]/g,'')" required step="0.0001" min="0" max="1000000">
												<div class="input-group-append"> <span class="input-group-text">{{$coinone}}</span> </div>
											</div>
										</div>
										<div class="form-group">
											<div class="control-value-box stoplimtboxt">
												...
												<div>
													<div class="row link-div">
														<div class="col" onclick="calculateBuyMarketAmount('25')">25%</div>
														<div class="col" onclick="calculateBuyMarketAmount('50')">50%</div>
														<div class="col" onclick="calculateBuyMarketAmount('75')">75%</div>
														<div class="col" onclick="calculateBuyMarketAmount('100')">100%</div>
													</div>
												</div>
											</div>
										</div>
										<div class="text-center">
											<input type="button" name="" id="buymarket_submit" class="btn btn-block sitebtn green-btn" value="Buy" /> </div>
									</form>
								</div>
								<div class="sellorderform">
									...
									<div class="form-group"><span> Available Balance: <span class="balanceOne">{{ $coinonebalance }}</span> {{$coinone}}</span></div>
										...
										<div class="form-group">
											<div class="input-group">
												 <div class="input-group-prepend"> <span class="input-group-text">Amount</span> </div> 
												<input type="number" class="form-control" placeholder="" onkeypress="return AvoidSpace(event)" class="form-control" id="sellmarketvolume" name="sellmarketvolume" onkeyup="if (/[^0-9.]/g.test(this.value)) this.value = this.value.replace(/[^0-9.]/g,'')" required step="0.0001" min="0" max="1000000">
												<div class="input-group-append"> <span class="input-group-text">{{$coinone}}</span> </div>
											</div>
										</div>
										<div class="form-group">
											<div class="control-value-box stoplimtboxt">
												...
												<div>
													<div class="row link-div">
														<div class="col" onclick="calculateSellMarketAmount('25')">25%</div>
														<div class="col" onclick="calculateSellMarketAmount('50')">50%</div>
														<div class="col" onclick="calculateSellMarketAmount('75')">75%</div>
														<div class="col" onclick="calculateSellMarketAmount('100')">100%</div>
													</div>
												</div>
											</div>
										</div>
										<div class="text-center">
											<input type="button" name="" class="btn btn-block sitebtn red-btn" id="sellmarket_submit" value="Sell" /> </div>
									</form>
								</div>
							</div>
						</div>
				...
				<div class="openorder">
					<div class="innerpagetab historytab">
						<ul class="nav nav-tabs tabbanner" role="tablist">
							...
							<li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#tradehistory" data-bs-target="#tradehistory">My Order History</a></li>
						</ul>
					</div>
					<div class="tab-content">
...
<script>
	...
	function loadOrderbook(data){
		...
		$.each(data.bids, function(k, v){
			...
			bR =  '<tr><td><span class="t-green">'+v[0]+'</span></td><td class="text-right">'+v[1]+'</td><td class="text-right">'+financial(bSum,coinTwoDecimal)+'</td></tr>'+ bR;
		});
		$.each(data.asks, function(k, v){
			let aSum = parseFloat(v[0]) * parseFloat(v[1]);
			aR =  '<tr><td><span class="t-red">'+v[0]+'</span></td><td class="text-right">'+v[1]+'</td><td class="text-right">'+financial(aSum,coinTwoDecimal)+'</td></tr>'+ aR;
		});
...
@elseif($selectPair->is_dust == 1)
	...
	var ordered = {};
	var newU = false;
	var selectPair = "{{ strtolower($coinone.$cointwo) }}";
	
	
	function loadOrderbook(data){
		depthSocketBuffer=data.depth;
	...
	websocket.onmessage = function(event) {
		...
		if(Data.e == 'depthUpdate'){
			...
			$.each(bufferB, function(k, v){
				...
				bR =  '<tr onclick="javascript:buyRow('+k+','+v+');"><td><span class="t-green">'+k+'</span></td><td class="text-right">'+v+'</td><td class="text-right">'+financial(bSum,coinTwoDecimal)+'</td></tr>'+ bR;
			});
			$.each(bufferA, function(k, v){
				let aSum = parseFloat(k) * parseFloat(v);
				//aR = aR + '<tr><td>'+k+'</td><td>'+v+'</td><td>'+financial(aSum)+'</td></tr>';
				aR =  aR + '<tr onclick="javascript:sellRow('+k+','+v+');"><td><span class="t-red">'+k+'</span></td><td class="text-right">'+v+'</td><td class="text-right">'+financial(aSum,coinTwoDecimal)+'</td></tr>'; 
			}); 
		...
		if((typeof Data['e'] == "aggTrade") || (Data['e'] != null))
		{
			if(Data['e'] == "aggTrade")
			{
				...
				com_order_data = ('<tr class="tr-div"><td><span class="'+is_data+'">'+parseFloat(Data['p']).toFixed(8)+'</span></td><td>'+parseFloat(Data['q']).toFixed(8)+'</td> <td>'+total.toFixed(8)+'</td> <td>'+s+'</td></tr>');
...
function update_openorders(){
  $.ajax({url:"{{url('/openorders')}}", type:"GET" ,async:true, cache:false ,success:function(result){
      $("#openorders").html(result);
  }});
}
function update_orderhistory(){
  $.ajax({url:"{{url('/orderhistory')}}", type:"GET" ,async:true, cache:false ,success:function(result){
      $("#orderhistory").html(result);
  }});
}
function update_tradehistory(){
  $.ajax({url:"{{ url('/tradehistoryajax/'.\Crypt::encrypt($selectPair->id)) }}", type:"GET", async:true, cache:false, success:function(result)
  {
