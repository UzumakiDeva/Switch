...
class P2PMarketplaceController extends Controller
{
    ...
    public function p2pTradeCancel(Request $request){
        ...
        try
        {
            ...
            $is_match = MarketPlace::where(['tuid' => $tuid])->whereIn('status',[0,1])->first();
    ...
    {
        $this->validate($request, [
            'xid' => 'required|alpha_dash|max:25',
        ]);
    ...
    public function buyMarketCreate(Request $request){
        ...
        try
        {
            ...
            $this->checkMatchSell($id,$buypair,$buyprice,$buyvolume);
            DB::commit();
    ...
    public function checkMatchSell($tid,$PID,$price,$amount){
        $pairID = (int)$PID;
        $price = (float)$price;
        $amount = (float)$amount;
    ...
    public function checkMatchBuy($tid,$PID,$price,$amount){
        $pairID = (int)$PID;
        $price = (float)$price;
        $amount = (float)$amount;
    ...
    public function sellMarketCreate(Request $request){
        ...
        try
        {
            ...
            $data['url'] = url('/p2p-matchorder/'.$order_id);
            $data['msg'] = "<div class='alert alert-success   alert-dismissible'>Post Trade Successfully!</div>";
            $this->checkMatchBuy($id,$pairID,$price,$volume);
            DB::commit();
            ...
            if($status  == 7){
                ...
                try {
                    Mail::send('email.p2p.dispute-admin', ['user' => $user], function($message) use ($user) {
                        ...
                        $message->to('support@pantherexchange.io');
                    });
