...
class WalletController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('wallet', ['balance' => $currency,'coins' => $coins]);
    }
    ...
    public function withdraw($coin)
    {
        ...
        if(is_object($coins)){
            ...
            return view('withdraw', ['address' => $address,'coinLists' => $coinLists,'coinname' => $coinname,'coin' => $symbol,'type' => $type,'coindetails' => $coins,'payment_id' => $payment_id,'balance' => $balance]);
        }else{
            return redirect('wallet')->with('adminwalletbank','Invalid Coin/Currency');
        }
    }
    public function validatecryptoWithdraw(Request $request){
        ...
        if(Auth::user()->kyc_verify == 1){
            ...
            if($btcDeatils){
                ...
                else if($currency == 'ETH' || $currency == 'BNB' || $currency == 'MATIC' || $btcDeatils->type == 'token')
                {
                    $this->validate($request, [
                ...
                if($debitamt > $fee){
                    if($amount >= $min_withdraw AND $amount > 0){
                        if($findminmumwithdrawamount >= $amount){   
                            if($to_address!=""){
                                if($balance >= $amount){
                                    ...
                                    if($twofa_withdraw_enable == 1 && Auth::user()->twofa !="")
                                    {   
                                    ...
                                    {
                                        \Mail::send('email.withdrawrequest', ['data' => $data], function($message) use ($user) {                                   
                                            $message->subject("Confirm Your Withdrawal Requested");
                        ...
                        return redirect('withdraw/'.$currency)->with('fail', "Withdraw amount must be greater than or equal to ".display_format($findminmumwithdrawamount,8)."  ".$currency); 
                    }  
                }else{
                    return redirect('withdraw/'.$currency)->with('fail', "Minimum ".$currency." withdraw amount should be greater than or equal to ".$findminmumwithdrawamount ."");
                }
            }else{
                return redirect('withdraw/'.$currency)->with('fail', "Invalid Coin/Currency given please check!"); 
            }
        }else{
            return redirect('withdraw/'.$currency)->with('fail', "Only KYC verified Users can Withdraw!"); 
        }         
                    ...
                    if($balance >= $amount){
                       $wallet = Wallet::where([['uid', '=', $uid],['currency',$currency]])->first();
                        if($type == 'fiat'){
                            ...
                            $internal_trans = Wallet::where([['uid' ,'!=' , $uid],['currency' ,'=' ,$currency],['mukavari' ,'=' , $to_address]])->first();
                            if(isset($internal_trans))
                            {
                                ...
                                $inserid = Coinwithdraw::createTransaction($uid,$currency,$fromaddress,$to_address,$amount,$debitamt1,$debitamt,1,$ses['network'],$ses['destination_tag']);

                                Wallet::creditAmount($uid1, $currency, $amount, 8,'withdraw',$remark,$inserid);

                            }else{
                                $inserid = Coinwithdraw::createTransaction($uid,$currency,$wallet->mukavari,$to_address,$amount,$debitamt1,$debitamt,0,$ses['network'],$ses['destination_tag']);
                            }
                        }                        
                        $remark = "User ".$currency." withdraw request";
                ...
                if($amount >= $min_withdraw AND $amount > 0){
                    if($to_address!=""){
                        if($balance >= $amount){
                            ...
                            if($type == 'fiat'){
                                $insertid = CurrencyWithdraw::createTransaction($uid,$currency,$ses['bankid'],$to_address,$amount,$debitamt1,$debitamt);
                               Wallet::debitAmount($uid,$currency,$amount,$decimal,'withdraw',$remark,$insertid);
                                ...
                                if(isset($internal_trans))
                                {
                                    ...
                                    $insertid = Coinwithdraw::createTransaction($uid,$currency,$fromaddress,$to_address,$amount,$debitamt1,$debitamt,1,$ses['network'],$ses['destination_tag']);
                                    ...
                                    $insertid = Coinwithdraw::createTransaction($uid,$currency,$wallet->mukavari,$to_address,$amount,$debitamt1,$debitamt,0,$ses['network'],$ses['destination_tag']);
